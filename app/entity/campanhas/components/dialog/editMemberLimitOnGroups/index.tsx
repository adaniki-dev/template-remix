...
export default function EditMemberLimitOnGroupsDialog() {
  ...
  return (
    <Dialog open={openModal} onOpenChange={handleCloseModal}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Atualize limite de membros dos grupos</DialogTitle>
          <DialogDescription>Recomendamos o limite ter capacidade de 600 membros</DialogDescription>
        </DialogHeader>
        <MyForm
          initialValues={{
            memberLimit: '',
          ...
          onSubmit={async (values, actions) => {
            ...
            const groupsWithUserLimit = memberLimitGroups?.groupIds.map((groupId) => ({
              id: groupId,
              memberLimit: Number(values.memberLimit),
            }));
            const body = {
              campaignsId,
              groups: groupsWithUserLimit,
            };
            await editGroups(body, {
              onSuccess: async () => {
                handleCloseModal();
              },
              onSettled: () => {
